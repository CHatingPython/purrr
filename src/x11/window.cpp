...
inline namespace x11 {
  ...
  void Window::setPosition(const std::pair<int, int> &position) {
    ::XMoveWindow(mContext->getDisplay(), mWindow, position.first, position.second);
  }
  ...
  void Window::setTitle(const char *title, int titleLength) {
    ...
    ::XChangeProperty(
        mContext->getDisplay(),
        mWindow,
        mContext->getNetWmNameAtom(),
        mContext->getUtf8StringAtom(),
        8,
        PropModeReplace,
        reinterpret_cast<const uint8_t *>(title),
        titleLength);
