...
void Context::fillKeyCodeTable() {
  ...
  std::pair<KeyCode, const char*> keymap[] = {
    { KeyCode::GraveAccent, "TLDE" },
    { KeyCode::N1, "AE01" },
    { KeyCode::N2, "AE02" },
    { KeyCode::N3, "AE03" },
    { KeyCode::N4, "AE04" },
  ...
  for (uint16_t scancode = desc->min_key_code; scancode <= desc->max_key_code; ++scancode) {
    ...
    for (int i = 0; i < desc->names->num_key_aliases; i++) {
      ...
      if (std::strncmp(desc->names->key_aliases[i].real, desc->names->keys[scancode].name, XkbKeyNameLength) != 0) continue;
...
void Context::handleEvent(::XEvent event) const {
  ...
  case ConfigureNotify: {
    if (event.xconfigure.width != window->mWidth ||
        event.xconfigure.height != window->mHeight) {
      window->inputWindowResize(
          window->mWidth = event.xconfigure.width,
          window->mHeight = event.xconfigure.height);
    }

    int xPos = event.xconfigure.x;
    int yPos = event.xconfigure.y;
    if (xPos != window->mXPos || yPos != window->mYPos) {
      window->inputWindowMove(
          window->mXPos = xPos,
          window->mYPos = yPos);
    }
  } break;
  ...
  case KeyRelease: {
    ...
    if (::XEventsQueued(mDisplay, QueuedAfterReading)) {
      ...
      if (next.type == KeyPress &&
          ...
          (next.xkey.time - event.xkey.time) < 20) break;
    }
